...
\begin{enumerate}
	...
	\end{enumerate}
	\item \textbf{Prop. } Let $S: V \to W$ be a linear map between inner product spaces. Then $S$ is an isometry iff all singular values of $S$ are 1. 	
	\item \textbf{Theorem. } If the positive singular values of $T: V \to W$ are $s_1,\dots,s_m$, then there are orthogonal lists $e_1,\dots,e_m \in V$ and $f_1,\dots,f_m \in W$ so that $Tv = s_1\langle v,e_1 \rangle f_1 + \dots + s_m \langle v,e_m \rangle f_m$ for all $v \in V$. 
	\item \textbf{Prop. } Let $T: V \to W$ be a linear map. If $s_1$ is the largest singular vlaue of $T$, then $||Tv|| \leq s_1 ||v||$ for all $v \in V$. 
\end{enumerate}
